...
func (s * ClusteredAppState) String() string { return "{ not implemented the String() function for ClusteredAppState correctly }" }
...
func (s * ClusteredAppState) populateNodes() error {
	...
	case "k8s":
		...
		if !ok { return fmt.Errorf("mode:cluster, but s.SD not correctly typed")}
